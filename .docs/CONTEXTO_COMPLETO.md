...
async def get_modal_inbox_mover(
    ...
    """Retorna el modal para mover una nota del inbox"""
    await init_rls(session, str(user.id))
    
    # Obtener la nota
    stmt = select(Anotacion).where(Anotacion.id == nota_id, Anotacion.user_id == user.id)
    ...
    # Obtener todas las categorías y cuadernos del usuario
    stmt = select(Categoria).where(Categoria.user_id == user.id).options(
...
<div class="w-full max-w-md bg-[#1a1d26] border border-white/10 rounded-2xl shadow-2xl overflow-hidden fade-in-up">
    
    <!-- Header (Mantenido) -->
    <div class="px-6 py-4 border-b border-white/5 flex justify-between items-center">
        <h3 class="text-sm font-bold text-white uppercase tracking-widest">Clasificar Nota</h3>
        <button @click="modalOpen = false" class="text-slate-500 hover:text-white"><i class="ph ph-x"></i></button>
    </div>

    <!-- Info Nota (MANTENIDO EL TÍTULO) -->
    <div class="px-6 py-3 bg-white/5 border-b border-white/5">
        <p class="text-[10px] text-slate-500 uppercase font-bold mb-0.5">Título actual</p>
        <p class="text-xs text-slate-300 italic truncate">"{{ nota.titulo }}"</p>
    </div>

    <div class="p-6">
        <form hx-post="/inbox/mover/{{ nota.id }}" hx-indicator="#btn-confirm-{{ nota.id }}" hx-swap="none">
            <label class="block text-[10px] text-slate-500 uppercase font-bold mb-2">Mover a Tema</label>
            <select name="nuevo_tema_id" required class="w-full bg-[#0f1117] border border-white/10 rounded-lg px-3 py-2 text-xs text-slate-300 focus:border-indigo-500 outline-none mb-4">
            ...
            <button type="submit" id="btn-confirm-{{ nota.id }}" class="group w-full py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold rounded-lg transition-all flex items-center justify-center gap-2">
        ...
        <div class="border-t border-white/5 mt-6 pt-4">
            <button hx-delete="/inbox/eliminar/{{ nota.id }}" hx-confirm="¿Eliminar nota?" hx-indicator="this" hx-swap="none"
                    class="w-full py-2 bg-red-500/10 hover:bg-red-500 text-red-500 hover:text-white text-xs font-bold rounded-lg transition-all border border-red-500/20 flex items-center justify-center gap-2">
                <span class="htmx-indicator-hide">Eliminar Nota</span>
                <span class="htmx-indicator"><i class="ph-bold ph-circle-notch animate-spin text-lg"></i></span>
            </button>
        </div>
    </div>
</div>
